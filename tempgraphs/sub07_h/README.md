# Subject 07: continuous model-completed graphs

86 PNGs cover 43 retained recording intervals. 42 intervals have model-assisted provisional six-stage fits. rohan_3/01 is an interrupted 8.42-second recording and retains partial phase annotations. All numeric series have no blank values, including this interval.

The left insole in rohan_1 is wholly model-estimated because its source file is absent. All 16 FMG channels in rohan_13/06 are wholly model-estimated because there are no FMG samples in that interval. Their waveforms must not be represented as measurements.

Mixed single-start recordings and paired-trigger batches are retained. The rohan_3 FMG clock reset remains two separate epochs; they are not joined to invent a complete trial. The rohan_13 batch uses insole trigger correspondence where FMG trigger events are missing. The 43 retained intervals are not independently confirmed as 43 distinct protocol trials.
