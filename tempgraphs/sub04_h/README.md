# SUB04_H: complete graphs with provisional segmentation

Sub04_H: ashu_1 and ashu_21. Left-insole trigger chatter is not used to define trials. Right-insole clock sections are aligned separately. Two missing FMG pulses in ashu_1 are recovered only because both insoles corroborate them. Local interval 11 lacks a measured end: its context extends to just before the next start (77.54 seconds), possibly including inter-trial rest. Its phase fit does not establish a true trial-end time.
...
When the initial bilateral fit cannot support six phases, the fallback evaluates each insole independently. Its first loading peak must coincide with sustained FMG activity (allowing the 15-sample support of the activity window). The more regular eligible pressure-cycle sequence is used for a provisional cycle-feature change fit. This avoids selecting a limb merely because standing noise creates more peaks. The fallback is recorded in phase_boundaries_provisional.csv; it is not independent validation.
...
python -B generate_subjects01_to04.py --subject Sub04_H

Verify:
python -B generate_subjects01_to04.py --subject Sub04_H --verify
